...
async function POST(request, { params }) {
    ...
    try {
        ...
        if (qrCode.status === 'archived') {
            return (0, api_response_1.successResponse)({ message: 'QR code is already archived' });
        }
        if (qrCode.status !== 'active') {
            return (0, api_response_1.errorResponse)('Can only archive active QR codes', 400);
        }
        qrCode.status = 'archived';
        await qrCode.save();
        return (0, api_response_1.successResponse)({ message: 'QR code archived successfully' });
    }
    catch (error) {
        console.error('Archive QR code error:', error);
        return (0, api_response_1.errorResponse)('Failed to archive QR code', 500);
    }
}
